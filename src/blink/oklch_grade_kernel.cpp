kernel OKLCHGrade : ImageComputationKernel<ePixelWise>
{
  ...
  float3 linear_srgb_to_xyz(float3 rgb)
  {
    ...
    float x = 0.4123907992659595 * rgb.x + 0.3575843393838780 * rgb.y + 0.1804807884018343 * rgb.z;
    float y = 0.2126390058715104 * rgb.x + 0.7151686787677559 * rgb.y + 0.0721923153607337 * rgb.z;
    float z = 0.0193308187155918 * rgb.x + 0.1191947797946260 * rgb.y + 0.9505321522496606 * rgb.z;
    return float3(x, y, z);
  }
  ...
  float3 xyz_to_linear_srgb(float3 xyz)
  {
    ...
    float r =  3.2409699419045213 * xyz.x + -1.5373831775700935 * xyz.y + -0.4986107602930033 * xyz.z;
    float g = -0.9692436362808798 * xyz.x +  1.8759675015077206 * xyz.y +  0.0415550574071756 * xyz.z;
    float b =  0.0556300796969936 * xyz.x + -0.2039769588889766 * xyz.y +  1.0569715142428786 * xyz.z;
    return float3(r, g, b);
  }
  ...
  float3 xyz_to_oklab(float3 xyz)
  {
    ...
    float l =  0.8190224379967030 * xyz.x + 0.3619062600528904 * xyz.y + -0.1288737815209879 * xyz.z;
    float m =  0.0329836539323885 * xyz.x + 0.9292868615863434 * xyz.y +  0.0361446663506424 * xyz.z;
    float s =  0.0481771893596242 * xyz.x + 0.2642395317527308 * xyz.y +  0.6335478284694309 * xyz.z;

    float l_ = signed_cbrt(l);
    float m_ = signed_cbrt(m);
    float s_ = signed_cbrt(s);

    float L =  0.2104542683093140 * l_ + 0.7936177747023054 * m_ + -0.0040720430116193 * s_;
    float a =  1.9779985324311684 * l_ + -2.4285922420485799 * m_ +  0.4505937096174110 * s_;
    float b =  0.0259040424655478 * l_ + 0.7827717124575296 * m_ + -0.8086757549230774 * s_;
  ...
  float3 oklab_to_xyz(float3 lab)
  {
    ...
    float l_ = 1.0 * lab.x + 0.3963377773761749 * lab.y +  0.2158037573099136 * lab.z;
    float m_ = 1.0 * lab.x + -0.1055613458156586 * lab.y + -0.0638541728258133 * lab.z;
    float s_ = 1.0 * lab.x + -0.0894841775298119 * lab.y + -1.2914855480194092 * lab.z;

    float l = l_ * l_ * l_;
    float m = m_ * m_ * m_;
    float s = s_ * s_ * s_;

    float x =  1.2268798758459243 * l + -0.5578149944602171 * m + 0.2813910456659647 * s;
    float y = -0.0405757452148008 * l +  1.1122868032803170 * m + -0.0717110580655164 * s;
    float z = -0.0763729366746601 * l + -0.4214933324022432 * m +  1.5869240198367816 * s;
